  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}account/verify-reset-otp/`,
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 relative">
      ...
      <div className="right col-span-1 lg:col-span-2 xl:col-span-3 flex flex-col justify-center overflow-y-auto">
        <div
          className={cn(
            "max-w-[800px] w-full mx-auto px-4 py-8 sm:px-6 sm:py-10 md:px-8 md:py-12 lg:px-10 lg:py-14 xl:px-16 xl:py-20 2xl:px-20 2xl:py-20 space-y-8 sm:space-y-10 lg:space-y-14"
          )}
        >
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-between gap-3 sm:gap-4">
            ...
            <button
              ...
              className="cursor-pointer text-base sm:text-lg lg:text-xl inline-flex justify-center items-center gap-1 w-full sm:w-40 py-3 sm:py-3.5 lg:py-4 rounded-md bg-primary text-[#FDFDFD] group disabled:opacity-50 disabled:cursor-not-allowed"
            >
